...
__global__ void Kernel_A(double* a, double* b, double* c, int matrixSize) {
    int bx = blockIdx.x;
    int by = blockIdx.y;
    int tx = threadIdx.x;
    int ty = threadIdx.y;
...
void matrixMultiplication(int device, double* h_a, double* h_b, double* h_c, int numRows) {
    double* d_a;
    double* d_b;
    double* d_c;

    int size = N * numRows * sizeof(double);
...
int main() {
    ...
    double* h_c0 = (double*)malloc(N * N * sizeof(double));
    double* h_c1 = (double*)malloc(N * N * sizeof(double));
    ...
    for (int i = 0; i < numRowsGPU0; ++i) {
        memcpy(h_c + i * N, h_c0 + i * N, N * sizeof(double));
    }

    // Copy h_c1 to the lower part of h_c
    for (int i = 0; i < numRowsGPU1; ++i) {
        memcpy(h_c + (i + numRowsGPU0) * N, h_c1 + i * N, N * sizeof(double));
    }
